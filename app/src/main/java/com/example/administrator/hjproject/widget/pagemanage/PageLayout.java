...
public class PageLayout extends RelativeLayout {
 ...
 /*       if (emptyView != null) {
    ...
    public View setCurrentRetryMessageViewLayout(int imageRes, String retryMsg) {
        return setCustomRetryMessage(PageManager.BASE_RETRY_LAYOUT_ID, imageRes, retryMsg);
    }
    ...
    private View setCustomRetryMessage(int layoutId, int retryImage, String retryMsg) {
        View emptyView = mInflater.inflate(layoutId, this, false);
        if (retryImage > 0) {
            ImageView iamge = (ImageView) emptyView.findViewById(R.id.iv_error_image);
            iamge.setImageResource(retryImage);
        }
        TextView viewById = (TextView) emptyView.findViewById(R.id.tv_error_msg);
        if (TextUtils.isEmpty(retryMsg)) {
            viewById.setText("页面加载失败");
        } else {
            viewById.setText(retryMsg);
        }
        return setRetryView(emptyView);

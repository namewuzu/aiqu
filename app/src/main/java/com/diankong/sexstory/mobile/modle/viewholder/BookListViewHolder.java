...
public class BookListViewHolder extends BaseViewHolder<BookInfoPojo> {
    ...
    public BookListViewHolder(ViewGroup parent) {
    ...
    public void setData(final BookInfoPojo data) {
        super.setData(data);
        tv_title.setText(data.articlename);
        tv_introduc.setText(data.intro);
        tv_author.setText(data.author);
        tv_author.setVisibility(View.GONE);
        tv_num.setText(data.chapters + "章");
        tv_type.setText(data.typeName);
        GlideImageLoader.onDisplayImage(getContext(), iv_icon, Api.IMGURL + data.articleid + "/" + data.articleid + Api.IMGURL_FOOT1);
        imgUrl = Api.IMGURL + data.articleid + "/" + data.articleid + Api.IMGURL_FOOT1;
